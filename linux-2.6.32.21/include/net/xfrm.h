...
struct xfrm_state
{
	...
	atomic_t		refcnt;// 所有使用计数 xfrm_state_hold() __xfrm_state_put
	spinlock_t		lock;// 状态锁  
	...
	struct {
		u32		reqid;//请求ID 
		u8		mode;//模式: 传输(XFRM_MODE_TRANSPORT)/通道(XFRM_MODE_TUNNEL) 
		u8		replay_window;//回放窗口
		u8		 aalgo, ealgo, calgo;// 认证,加密,压缩算法ID值
		u8		flags;// 一些标准
		u16		family;//ipv4或ipv6
		xfrm_address_t	saddr;// 源地址
		int		header_len;// 添加的协议头长度
		int		trailer_len;
	} props;//状态属性

	struct xfrm_lifetime_cfg lft; //寿命
...
struct xfrm_policy
{
	...
	atomic_t		refcnt;//xfrm策略引用计数器 在xfrm_policy_alloc初始化为1
                          /*
                            xfrm_pol_put() 减一
                            xfrm_pol_hold() 加1操作
						  */

	struct timer_list	timer;//策略定时器 用于策略过期的情况 处理函数xfrm_policy_timer()

	u32			priority;//策略优先级 
	u32			index;   //策略索引号
	struct xfrm_selector	selector;//指定要将策略用于哪些流

							//ip -stat xfrm policy show	 查看寿命配置条目
	struct xfrm_lifetime_cfg lft;//策略的寿命
	struct xfrm_lifetime_cur curlft;//xfrm策略的当前寿命
	struct dst_entry       *bundles;// 路由链表 
	struct xfrm_policy_walk_entry walk;
	u8			type;//XFRM_POLICY_TYPE_MAIN
	u8			action; // 策略动作, 接受/加密/阻塞.
	u8			flags;
	u8			xfrm_nr;//与策略关联的模板数 最多为6 copy_templates()中进行初始化
	u16			family;//ipv4或ipv6
	struct xfrm_sec_ctx	*security;//一个安全上下文
	struct xfrm_tmpl       	xfrm_vec[XFRM_MAX_DEPTH];//xfrm状态和xfrm策略之间的过滤结构 
};
