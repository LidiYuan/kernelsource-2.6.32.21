...
struct inet_connection_sock {
	...
	//连接管理队列(包括以完成三次握手 和未完成三次握手)
	struct request_sock_queue  icsk_accept_queue;
	
	struct inet_bind_bucket	  *icsk_bind_hash;//关联绑定的端口
	
	unsigned long		  icsk_timeout;
 	struct timer_list	  icsk_retransmit_timer;//重传定时器 看inet_csk_init_xmit_timers
 	struct timer_list	  icsk_delack_timer;//ack延迟定时器 看inet_csk_init_xmit_timers
	__u32			  icsk_rto;
...
#define ICSK_TIME_DACK		2	/* Delayed ack timer ack延迟定时器*/
...
方等待对端的ACK，直到连接超时关闭。
为了避免上述情况的发生，发送方实现了一个零窗口探测定时器
*****************************************/
//当接收方的接收窗口为0时，每隔一段时间，发送方会主动发送探测包，通过迫使对端响应来得知其接收窗口有无打开
#define ICSK_TIME_PROBE0	3	/* Zero window probe timer 零窗口探测定时器*/
