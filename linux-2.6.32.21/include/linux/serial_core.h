...
struct uart_ops {
	...
	int		(*set_wake)(struct uart_port *, unsigned int state); /* 返回一描述串口类型的字符串 */
...
*/
struct uart_port {
	spinlock_t		lock;			/* 串口端口锁 *//* port lock */
	unsigned long		iobase;			 /* IO端口基地址 *//* in/out[bwl] */
	unsigned char __iomem	*membase;		/* IO内存基地址,经映射(如ioremap)后的IO内存虚拟基地址 *//* read/write[bwl] */
