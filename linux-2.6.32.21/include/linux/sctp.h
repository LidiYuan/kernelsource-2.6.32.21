...
typedef struct sctphdr {
	__be16 source;//sctp源端口
	__be16 dest; //sctp目标端口
	__be32 vtag; //验证标签 是一个随机值
	__le32 checksum;//sctp通用报头和所有块的校验和
...
typedef struct sctp_chunkhdr {
	__u8 type; //sctp块的类型 数据块的类型为SCTP_CID_DATA
	__u8 flags;//通常发送方将其置为0 而接收方忽略此值  如果发送发填充了验证标签将会把她设置为0 否则是1
	__be16 length;//sctp块的长度
