...
#define HRTIMER_STATE_ENQUEUED	0x01 //定时器已经被放入红黑树
 ...
 */
struct hrtimer {
	struct rb_node			node; //用于红黑树管理
	ktime_t				_expires; //定时器到期时间
	ktime_t				_softexpires;

	//返回值为一个枚举 它决定了hrtimer是否重新激活HRTIMER_RESTART
	enum hrtimer_restart		(*function)(struct hrtimer *);//定时器函数
	struct hrtimer_clock_base	*base;
	unsigned long			state; //定时器状态 HRTIMER_STATE_INACTIVE
#ifdef CONFIG_TIMER_STATS
	int				start_pid;  //创建该定时器的进程pid
	void				*start_site;
	char				start_comm[16];//创建定时器的进程名
#endif
};
 ...
 */
 //每个cpu有一个hrtimer_cpu_base结构 hrtimer_cpu_base管理着2种不同的时间基准系统的hrtimer 实时时间和相对时间 
struct hrtimer_cpu_base {
	spinlock_t			lock;
