...
//端点描述符 
///USB 端点描述符(每个USB设备最多有16个端点)
struct usb_endpoint_descriptor {
	__u8  bLength;//描述符的字节长度
	__u8  bDescriptorType; //描述符类型，对于端点就是USB_DT_ENDPOINT

	__u8  bEndpointAddress;//bit0~3表示端点地址，第7位是方向（0为out，1为in）
	__u8  bmAttributes;	   //属性(bit0、bit1构成传输类型，00--控制，01--等时，10--批量，11--中断)
                           /*
                             控制：控制端点用来控制对USB设备的不同部分的访问。它们通常用于配置设备、获取设备信息、发送命令到设备，或者获取设备的状态报告
                             中断: 每当USB Host要求设备传输数据时，中断端点就以一个固定的速率来传送少量的数据。这些端点是USB键盘和鼠标所使用的主要传输方式
                             批量: 批量（bulk）端点传送大批量的数据。这些端点通常比中断端点大得多（它们可以一次持有更多的字符）。它们常见于需要确保没有数据丢失的传输的设备 
                             等时: 等时（isochronous）端点同样可以传送大批量的数据，但数据是否达到是没有保证的(例如音频和视频设备)
						   */
