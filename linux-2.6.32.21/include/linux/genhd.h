...
struct hd_struct 
{
	...
	sector_t nr_sects; /*扇区数，512 字节为1 个扇区*/
...
//通用磁盘对象 将直接被块设备驱动程序分配和操控
//对一个块设备及其可能的若干个分区设备来说 gendisk对象只有一个
//而block_device 对象每个分区有一个
struct gendisk {
	...
	int major;			/***************
	                      主设备号 用于指定当前设备所对应的驱动程序  次设备号对应该驱动程序所管理的块设备上的一个分区
                          同一个磁盘的各个分区共享一个主设备号，而次设备号则不同
                          在linux上磁盘上的一个独立分区被看做一个设备 对应/dev下的一个节点
	                    **********************/

    /*first_minor和 minor表示从设备号的范围  从设备代表一个分区*/
	...
	const struct block_device_operations *fops; /*块设备操作结构体*/

	//当前gendisk 对象所代表的块设备上的I/O请求队列
	struct request_queue *queue;
	
	void *private_data;/*私有数据 内核块子系统不会修改它*/

               //描述驱动器状态的标志
	int flags; //如果为可移动介质 则是 GENHD_FL_REMOVABLE
	           //如果为CD_ROM    则是GENHD_FL_CD
