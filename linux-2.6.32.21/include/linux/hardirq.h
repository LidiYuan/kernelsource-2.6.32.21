...
#define NMI_OFFSET	(1UL << NMI_SHIFT)         // 1<<26
 ...
 */
#define in_irq()		(hardirq_count())//CPU正服务于硬件中断时，返回True
#define in_softirq()		(softirq_count())//CPU正服务于软件中断时，返回True
#define in_interrupt()		(irq_count()) //CPU正在服务于一个硬件中断或软件中断。或抢占功能关闭时，返回True
 ...
 */
 //当调度不被允许此值返回 非0值  
 //不允许调度:{在中上下文(软硬) 拥有自旋锁}
 //拥有自旋锁 此返回1 若没有开启抢占功能(CONFIG_PREEMPT)， 自旋锁下就会返回0
