...
//何时终止对感兴趣队列中注册函数的调用
#define NOTIFY_DONE		0x0000		/* 对通知信息不敢兴趣*/
#define NOTIFY_OK		0x0001		/* 通知信息被正确处理*/
#define NOTIFY_STOP_MASK	0x8000		/* 用于检查是否停止调用回调函数*/
#define NOTIFY_BAD		(NOTIFY_STOP_MASK|0x0002)//某个感兴趣者出现问题 停止往下通知
						/* Bad/Veto action */
/*
 * Clean way to return from the notifier and stop further calls.
 */
#define NOTIFY_STOP		(NOTIFY_OK|NOTIFY_STOP_MASK)//被正确处理 此事件不必进一步调用其他回调
...
#define CPU_ONLINE		0x0002 /* CPU (unsigned)v is up CPU就绪*/
#define CPU_UP_PREPARE		0x0003 /* CPU (unsigned)v coming up  CPU运行起来但还没有就绪*/
#define CPU_UP_CANCELED		0x0004 /* CPU (unsigned)v NOT coming up  热插拔*/
#define CPU_DOWN_PREPARE	0x0005 /* CPU (unsigned)v going down */
#define CPU_DOWN_FAILED		0x0006 /* CPU (unsigned)v NOT going down */
#define CPU_DEAD		0x0007 /* CPU (unsigned)v dead 热插拔*/
