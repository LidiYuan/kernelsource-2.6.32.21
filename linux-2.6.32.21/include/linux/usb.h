...
struct usb_host_endpoint {
	struct usb_endpoint_descriptor	desc;//包含一个端点描述符
	struct list_head		urb_list; //端点要处理的urb队列 urb是什么？urb可是usb通信的主角，它包含了执行urb传输所需要的所有信息，若要进行usb通信，就得创建一个urb，并且为它赋好值（专业点就是初始化），交给咱们的usb core，它会找到合适的host controller，从而进行具体的数据传输。设备中的每个端点都可以处理一个urb队列，可以理解为urb是内核里对usb传输数据的封装（或者抽象）
	void				*hcpriv;//这是提供给HCD（host controller driver）用的。比如等时端点会在里边儿放一个ehci_iso_stream
	struct ep_device 		*ep_dev;	/* For sysfs info这个字段是供sysfs用的 
...
struct usb_host_interface {
	struct usb_interface_descriptor	desc;//包含一个接口描述符

	/* array of desc.bNumEndpoint endpoints associated with this
	 * interface setting.  these will be in no particular order.
	 */
	struct usb_host_endpoint *endpoint;//端点结构数组
...
struct usb_interface {
	...
	struct usb_host_interface *altsetting;//设置结构数组 因为一个接口可以有多个设置
...
struct usb_host_config {
	...
	char *string;		/* iConfiguration string, if present  这个字符串保存了配置描述符iConfiguration字段对应的字符串描述符信息*/
...
struct usb_bus {
    ...
    /*
	在usb世界里，一条总线就是一棵大树，一个设备就是一片叶子。为了记录这棵树上的每一个叶子节点，每条总线设有一个地址映射表，即struct usb_bus结构体里有一个成员struct usb_devmap devmap
					 */
...
/*
一个主控制器对应一条usb总线，一个主控制器绑定着一个root hub，一个root hub对应于一个usb_device
usb_new_device()
...
struct usb_device {
	int		devnum; //设备的地址。这和咱们写程序时说的地址是不一样的，devnum只是usb设备在一条usb总线上的编号
                    /*
                       你的usb设备插到hub上时，hub响应就会调用一个名叫choose_address的函数（在hub.c文件里），为你的设备选择一个地址
                       
					*/	
	char		devpath [16];//级联的端口号 
	                         
	u32		route;
	enum usb_device_state	state; //设备的状态
	enum usb_device_speed	speed;//设备的速度

	struct usb_tt	*tt;//transaction translator。你可以把它想成一块特殊的电路，是hub里面的电路，确切的说是高速hub中的电路
	                    //如果一个高速设备里有这么一个TT，那么就可以连接低速/全速设备，如不然，那低速/全速设备没法用，只能连接到OHCI/UHCI那边出来的hub口里
	int		ttport;

	unsigned int toggle[2];//分别对应IN和OUT端点，每一个端点占一位
