 ...
 * IRQ types
 */
#define IRQ_TYPE_NONE		0x00000000	/* 系统默认没有指定类型的触发模式Default, unspecified type */
...
#define IRQ_TYPE_EDGE_BOTH (IRQ_TYPE_EDGE_FALLING | IRQ_TYPE_EDGE_RISING)//上升沿和下降沿两种触发
#define IRQ_TYPE_LEVEL_HIGH	0x00000004	/* Level high type *///高电平触发
#define IRQ_TYPE_LEVEL_LOW	0x00000008	/* Level low type *///低电平触发
#define IRQ_TYPE_SENSE_MASK	0x0000000f	/* Mask of the above *///以上任何一种条件
#define IRQ_TYPE_PROBE		0x00000010	/* Probing in progress *///在进程中查询

/* Internal flags */
#define IRQ_INPROGRESS		0x00000100	/* IRQ handler active - do not enter!  正在执行这个IRQ的一个处理程序*/
#define IRQ_DISABLED		0x00000200	/* IRQ disabled - do not enter! 由设备驱动程序已经禁用了这条IRQ中断线*/
#define IRQ_PENDING		0x00000400	/* IRQ pending - replay on enable 一个IRQ已经出现在中断线上，且被应答，但还没有为它提供服务*/
#define IRQ_REPLAY		0x00000800	/* IRQ has been replayed but not acked yet 继续中断处理*/
#define IRQ_AUTODETECT		0x00001000	/* IRQ is being autodetected 自动检测中断请求 */
#define IRQ_WAITING		0x00002000	/* IRQ not yet seen - for autodetection 对自动检测中断 此时还没有看到中断到来 */
#define IRQ_LEVEL		0x00004000	/* IRQ level triggered */
#define IRQ_MASKED		0x00008000	/* IRQ masked - shouldn't be seen again屏蔽改中断 */
#define IRQ_PER_CPU		0x00010000	/* IRQ is per CPU 每个cpu都有一个IRQ*/
...
/*
linux内核将所有的中断统一编号，使用一个irq_desc[NR_IRQS]的结构体数组来描述这些中断：
每个数组项对应着一个中断源（可能是一个中断，也可能是一组中断），
记录了中断的入口处理函数（不是用户注册的处理函数）、中断标记，并提供了中断的底层硬件访问函数
