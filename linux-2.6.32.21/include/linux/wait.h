...
//等待队列项
struct __wait_queue{
	unsigned int flags;//唤醒进程的方式 其中0是非互斥，WQ_FLAG_EXCLUSIVE(0×01)是互斥
#define WQ_FLAG_EXCLUSIVE	0x01
	void *private;//私有指针变量，使用过程中会用来存放task_struct结构体
	wait_queue_func_t func;// func是一个函数指针，指向用于唤醒队列中线程的函数。虽然提供了默认的唤醒函数default_wake_function，但也允许灵活的设置队列的唤醒函数
...
//函数也和add_wait_queue()函数功能基本一样，只不过它是将等待的进程(wait)设置为互斥进程
...
//排他性的等待,排他性解释:
//有一些进程都在等待队列中,当唤醒的时候，唤醒所有的非排他性进程和一个排他性进程
