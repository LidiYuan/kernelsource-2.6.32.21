...
try_again:
	...
	while ((offset -= size) >= 0) {
		...
		//设置缓冲区首部的一些字段，主要是b_page字段（指向页描述符）和b_data字段
		/* Link the buffer to its page */
		set_bh_page(bh, page, offset);

		//设置b_end_io字段和b_private字段
...
*/
static struct page *grow_dev_page(struct block_device *bdev, sector_t block,
		pgoff_t index, int size)
{
	...
	//该函数主要是在页高速缓存中查找相应的缓冲区页是否存在，如果不存在则建立相应的缓冲区页。最终返回相应缓冲区页的页描述符的地址
	...
	//该函数主要是检查page中的PG_private标志，如果该标志位空的说明，该页不是一个缓冲区页。那么这个时候接下来就会分配相应的缓冲区首部
	if (page_has_buffers(page)) 
	{
		//主要是根据页描述符page的private字段来获取缓冲区页的第一个缓冲区首部的地址，即获取第一缓冲区首部bh，这是在内存中有相应的缓冲区页的情况下才执行这个函数
		...
		//如果页中块的大小有错误，就调用try_to_free_buffers()释放缓冲区页的上一个缓冲区首部
	 ...
	 */
	//根据页中所请求的块大小为页分配n个缓冲区首部，并把他们通过缓冲区首部的字段b_this_page连接成单向循环链表，同时设置各个缓存区首部的相应的字段的值 
	...
	//该函数主要是把缓冲区头部连成一个循环链表
	link_dev_buffers(page, bh);

	//该函数的功能是初始化缓冲区首部的其他字段的值
	init_page_buffers(page, bdev, block, size);
 ...
 */
 //在页高速缓存中搜索块
 //block_device描述符的地址bdev,块号block,块大小size
struct buffer_head *__find_get_block(struct block_device *bdev, sector_t block, unsigned size)
{
	//先检查执行CPU的LRU块高速缓存数组中是否有这个缓冲区首部
	struct buffer_head *bh = lookup_bh_lru(bdev, block, size);

	if (bh == NULL) 
	{
		bh = __find_get_block_slow(bdev, block);
		if (bh)//把LRU块高速缓存中的所有元素向下移动一个位置，并把指向所请求块的缓冲区首部的指针插入到第一个位置
			bh_lru_install(bh);
	}
...
{
//调用__find_get_block()检查块是否已经在页高速缓存中。如果找到块，则函数返回其缓冲区首部的地址。
	struct buffer_head *bh = __find_get_block(bdev, block, size);

// 否则，调用__getblk_slow，触发grow_buffers()为所请求的页分配一个新的缓冲区页
	might_sleep();
	if (bh == NULL)
...
struct buffer_head *__bread(struct block_device *bdev, sector_t block, unsigned size)
{
	...
	//如果块已经在页高速缓存中并包含有效数据（if(buffer_uptodate(bh))检查BH_Uptodate标志被置位），就返回缓冲区首部的地址
	//否则__bread()从磁盘读块
	...
	for (i = 0; i < nr; i++) {
		...
		if (rw == WRITE || rw == SWRITE || rw == SWRITE_SYNC ||
			...
			if (test_clear_buffer_dirty(bh)) {
				...
				get_bh(bh);
				if (rw == SWRITE_SYNC)
					submit_bh(WRITE_SYNC, bh);///提交WRITE写标志的buffer_head 
				else
					submit_bh(WRITE, bh);
				continue;
			}
