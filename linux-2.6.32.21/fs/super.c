	                      ...
	                      struct vfsmount *mnt)
{
	...
	s = sget(fs_type, compare_single, set_anon_super, NULL);//find or create a superblock
	if (IS_ERR(s))
		return PTR_ERR(s);
	
	if (!s->s_root) //是否有根dentry
	{
		s->s_flags = flags;
		//填充sb 对于sysfs是sysfs_fill_super
		error = fill_super(s, data, flags & MS_SILENT ? 1 : 0);
		if (error) 
		{
			deactivate_locked_super(s);
...
{
    //下面的type是static struct file_system_type  rootfs_fs_type
