...
static irqreturn_t rtc_interrupt(int irq, void *dev_id)
{
	 ...
	 */
    //调用自旋锁 保证rtc_irq_data 不被SMP机器上的其他处理器同时访问
	spin_lock(&rtc_lock);

	//rtc_irq_data存放RTC的信息 每次中断都会更新 反应中断的状态
	rtc_irq_data += 0x100;
	rtc_irq_data &= ~0xff;
	...
	//调用自旋锁 保证rtc_callback不被SMP机器上的其他处理器同时访问
	spin_lock(&rtc_task_lock);
	if (rtc_callback)
		rtc_callback->func(rtc_callback->private_data);//rtctimer_interrupt()
