...
struct arp_packet {
	u8 dest_mac[ETH_ALEN];//接收方MAC
	u8 src_mac[ETH_ALEN];//发送方MAC
	__be16 type;//0x0806是ARP帧的类型值
	__be16 ar_hrd;//硬件类型 - 以太网类型值0x1
	__be16 ar_pro;//上层协议类型 - IP协议(0x0800)
	u8 ar_hln;//MAC地址长度
	u8 ar_pln;//IP地址长度
	__be16 ar_op;//操作码 - 0x1表示ARP请求包,0x2表示应答包
	u8 ar_sha[ETH_ALEN];//发送方mac
	u8 ar_sip[4];//发送方ip
	u8 ar_tha[ETH_ALEN];//接收方mac
	u8 ar_tip[4];//接收方ip
} __attribute__ ((__packed__));
