...
static void e1000_netpoll(struct net_device *netdev)
{
	...
	e1000_intr(adapter->pdev->irq, netdev);//这是网卡驱动的中断处理函数，也就是后半段的处理
				 ...
				 const struct pci_device_id *ent)
{
	...
	netdev->netdev_ops		= &e1000e_netdev_ops;//网卡设备注册的操作函数
...
static int __init e1000_init_module(void)
{
	...
	printk(KERN_INFO "%s: Copyright (c) 1999-2008 Intel Corporation.\n",
	       e1000e_driver_name);
	ret = pci_register_driver(&e1000_driver);//注册网卡驱动，按照PCI驱动开发方式来进行注册
										     //e1000_driver 驱动结构体
