...
vortex_interrupt(int irq, void *dev_id)
{
	...
	status = ioread16(ioaddr + EL3_STATUS);//读取寄存器从而得到当前的中断状态
	...
	do {
		...
		//RxComplete 表示有新帧到达 可以接收
		if (status & RxComplete)
			vortex_rx(dev);//用于接收数据包

        //TxAvailable 表示一个中断 设备内存有空间 用于传输
		if (status & TxAvailable) 
		{
			...
			//唤醒设备传输队列  将会触发传输软中断net_tx_action()
			netif_wake_queue (dev);
		}

        //接收到 cpu对设备的DMA传输已经完成  设备发此中断通知驱动
		if (status & DMADone) 
		{
			if (ioread16(ioaddr + Wn7_MasterStatus) & 0x1000) 
			{
				iowrite16(0x1000, ioaddr + Wn7_MasterStatus); /* 回复接收到此事件 */
				pci_unmap_single(VORTEX_PCI(vp), vp->tx_skb_dma, (vp->tx_skb->len + 3) & ~3, PCI_DMA_TODEVICE);

                //由于传输缓冲区传输到设备已经完成 这又在中断环境 采用此函数进行延迟释放缓冲区skb
		...
		if (--work_done < 0) //work_done存储最大可以处理帧的最大数目
		{
...
//参数为设备  此函数可以处理同种设备类型的设备实体  因此 函数启动必须知道是哪个设备
static int vortex_rx(struct net_device *dev)
{
	struct vortex_private *vp = netdev_priv(dev);
	void __iomem *ioaddr = vp->ioaddr;
	...
	while ((rx_status = ioread16(ioaddr + RxStatus)) > 0) 
	{
		...
		{
			/* The packet length: up to 4.5K!.获得封包的长度 */
			...
			if (skb != NULL) 
			{  
			    //以太网报头长14字节  要让ip报头为16字节对齐 需要偏移 16-14=2 字节
			    /*ip头按16字节边界对齐
...
static void set_rx_mode(struct net_device *dev)
{
	...
	//EL3_CMD是对ioaddr内存地址的偏移量 代表与设备交互时 命令拷贝到何处
