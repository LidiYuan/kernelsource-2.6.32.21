...
static int serio_connect_driver(struct serio *serio, struct serio_driver *drv)
{
	...
	retval = drv->connect(serio, drv);//调用驱动提供的connect  atkbd_connect
...
enum serio_event_type {
	SERIO_RESCAN_PORT, //重新扫描
	SERIO_RECONNECT_PORT,//
	SERIO_RECONNECT_CHAIN,
	SERIO_REGISTER_PORT,//端口注册事件
	SERIO_ATTACH_DRIVER,
};
			     ...
			     enum serio_event_type event_type)
{
	 ...
	 */
	 //遍历serio_event_list链表 检查所有注册的事件,同一个端口只能注册一个事件
	list_for_each_entry_reverse(event, &serio_event_list, node) {
		if (event->object == object) {
			if (event->type == event_type) //发现相同的event
				goto out;
			break;
		}
	}
	...
	wake_up(&serio_wait);//唤醒线程  这个事件是由serio_thread内核线程处理的
...
static void serio_handle_event(void)
{
	...
	if ((event = serio_get_event())) {
		...
		switch (event->type) {
			case SERIO_REGISTER_PORT: //注册端口事件
...
static int serio_thread(void *nothing)
{
	set_freezable();
	do {
		serio_handle_event();//处理事件函数

		//内核线程进入睡眠状态，如果被唤醒,且事件链表不是空,则处理事件
...
static void serio_init_port(struct serio *serio)
{
	...
	serio->dev.bus = &serio_bus; //设置总线类型为serio
...
static void serio_add_port(struct serio *serio)
{
	...
	if (serio->parent) {
		...
		serio->parent->child = serio;//修改端口父设备的参数
	...
	//通过总线提供的match函数找到一个合适的驱动然后调用总线的probe函数
...
void __serio_register_port(struct serio *serio, struct module *owner)
{
	...
	//注册一个SERIO_REGISTER_PORT 端口登记事件
