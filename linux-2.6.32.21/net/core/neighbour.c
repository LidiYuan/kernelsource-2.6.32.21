...
static struct neighbour *neigh_alloc(struct neigh_table *tbl)
{
	...
	//如果表条目大于gc_thresh3或者 条目大于gc_thresh2并且距离最后一次刷新的频率大于5hz
	//将调用同步垃圾收集器
	if (entries >= tbl->gc_thresh3 ||
	    (entries >= tbl->gc_thresh2 &&
	     time_after(now, tbl->last_flush + 5 * HZ))) 
	{
	     //若调用垃圾收集器后表条目还大于 gc_thresh3则不分配新的邻居对象 直接返回
	...
	n->nud_state	  = NUD_NONE;
	n->output	  = neigh_blackhole;//此方法丢弃数据包 返回ENETDOWN
			       ...
			       struct net_device *dev)
{
	...
	int key_len = tbl->key_len;
	int error;
	struct neighbour *n1, *rc, *n = neigh_alloc(tbl);//创建一个邻居对象,并进行初始化
...
static void neigh_timer_handler(unsigned long arg)
{
	...
	if (neigh->nud_state & (NUD_INCOMPLETE | NUD_PROBE)) {
		...
		write_unlock(&neigh->lock);
		//此处的ops->solicit为arp_solicit 来发送数据包
...
int __neigh_event_send(struct neighbour *neigh, struct sk_buff *skb)
{
	...
	if (neigh->nud_state == NUD_INCOMPLETE) 
	{
		if (skb) 
		{
			...
			//将skb加入到邻居队列中定时器neigh_timer_handler会将队列中的包发送出去
