...
@size:申请缓冲区的大小
@gfp_mask:申请码
@clone:是否申请一个可以被克隆的cache
*/

/*
__alloc_skb函数返回后skb的分配效果
			    ...
			    int fclone, int node)
{
	...
	skb_reset_tail_pointer(skb);//设置tail指针tail = skb->head
	
	skb->end = skb->tail + size; //end指向数据的结尾 但end并不是真正的数据结尾 后边还有skb_shared_info结构
 ...
 */
//只复制skb->start 到 skb->end之间的数据 注意和skb_copy()的区别
...
/*
//skbuff_head_cache高速缓存，一般情况下，SKB都是从该高速缓存中分配的
*/
	...
	/*
	//创建每次以两倍SKB描述符长度来分配空间的skbuff_fclone_cache高速缓存。如果在分配SKB时  
