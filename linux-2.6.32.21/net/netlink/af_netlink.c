...
static int netlink_create(struct net *net, struct socket *sock, int protocol)
{
	...
	//对于netlink只能使用SOCK_RAW 或 SOCK_DGRAM
...
static struct sock *netlink_getsockbypid(struct sock *ssk, u32 pid)
{
	...
	nlk = nlk_sk(sock);
	if (sock->sk_state == NETLINK_CONNECTED && nlk->dst_pid != nlk_sk(ssk)->pid) 
	{
		sock_put(sock);
		return ERR_PTR(-ECONNREFUSED);
	}
...
struct sock *netlink_kernel_create(struct net *net, //是一个网络名字空间namespace
		                           ...
		                           struct mutex *cb_mutex,            //互斥锁 可选若为NULL则使用netlink_sock中的cb_def_mutex
		                           struct module *module)
{
	struct socket *sock;
	struct sock *sk;
	...
	if (input)
		nlk_sk(sk)->netlink_rcv = input;//设置内核接收Netlink消息的函数，这里就是前面的rtnetlink_rcv函数
	...
	nlk->flags |= NETLINK_KERNEL_SOCKET; //设置标志 和应用层的区别
