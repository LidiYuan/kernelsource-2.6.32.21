...
/*
tcp_sendmsg中主要做的工作就是讲字节流分段(根据mss),然后传递给ip层
*/
		...
		size_t size)
{
	...
	flags = msg->msg_flags;///首先取出句柄的flag,主要是看是非阻塞还是阻塞模式. 
	timeo = sock_sndtimeo(sk, flags & MSG_DONTWAIT);///这里取得发送超时时间.
...
static void tcp_prequeue_process(struct sock *sk)
{
	 ...
	 * necessary */
	local_bh_disable();
	while ((skb = __skb_dequeue(&tp->ucopy.prequeue)) != NULL) //遍历链表tp->ucopy.prequeue
		sk_backlog_rcv(sk, skb);//调用函数tcp_v4_do_rcv将数据包加入sk->receive_queue
	local_bh_enable();
		...
		size_t len, int nonblock, int flags, int *addr_len)
{
	...
	//如果设置了MSG_WAITALL，则返回len, 若len=0 则返回1
	//如果没设置MSG_WAITALL，则返回sk_rcvlowat和len中较小者，若较小者为0则返回1
	...
	do {
		...
		if (tp->urg_data && tp->urg_seq == *seq) {//存在紧急数据,并且将要读取的序列号就是紧急数据序列号
			if (copied)//如果已将拷贝过数据,紧急数据在普通数据包中 则退出循环
				break;
			if (signal_pending(current)) {//若有未决信号
		...
		/* Next get a buffer. */
        //遍历sk_receive_queue队列
		skb_queue_walk(&sk->sk_receive_queue, skb) 
	   {
			/* Now that we have two receive queues this
			 * shouldn't happen.
			 */
			 //若要读取的序列号小于保中数据开始的序列号,说明出现序列号断层
			 //比如要读取的序列号为100  而数据包中序列号开始为110  则100-109序列号丢失
        ...
        //第一次执行此函数tp->ucopy.task和user_recv都为NULL
		...
		if (copied >= target) 
		{
			...
			release_sock(sk);//处理backlog队列中的包；这些包会进入tcp_v4_do_rcv函数
			lock_sock(sk);
		} else
			sk_wait_data(sk, &timeo);
		...
		if (user_recv) 
		{
            ...
            //接收缓存中没有数据 但prequeue中有数据
			if (tp->rcv_nxt == tp->copied_seq &&
			    !skb_queue_empty(&tp->ucopy.prequeue)) 
			{ 
do_prequeue:
				tcp_prequeue_process(sk);//将prequeue中的skb放入tcp_v4_do_rcv中进行处理
...
void tcp_close(struct sock *sk, long timeout)
{
	...
	lock_sock(sk);
	sk->sk_shutdown = SHUTDOWN_MASK;//表示双向关闭
	 ...
	 */
	 //清空接收缓冲区 将未上报给应用层的数据长度记录的data_was_unread中
	 ...
	 */
	 //如果接收队列中有数据位读取 这时内核发送给对端的不是FIN 而是RST
	if (data_was_unread) {
	...
	//若设置了SO_LINGER 并且linger{l_onoff=1  l_linger=0} 此时断开连接发送RST
