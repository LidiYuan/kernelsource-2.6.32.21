...
static u16 tcp_select_window(struct sock *sk)
{
	...
	tp->rcv_wnd = new_win;//保存接收窗口大小
	tp->rcv_wup = tp->rcv_nxt;//设置接收滑动窗口的左边沿
			                          ...
			                          gfp_t gfp_mask)
{
	...
	*(((__be16 *)th) + 6)	= htons(((tcp_header_size >> 2) << 12) |
    ...
    //在tcp_v4_init_sock()中将icsk_af_ops设置为ipv4_specific{}
    //queue_xmit为ip_queue_xmit()
...
{
/*
：在判断当前包是否可发送时，只需检查最近的一个小包是否已经确认,基于的原理很简单，
既然发送的小包都已经确认了，也就是说网络上没有当前连接的小包了，
所以发送一个即便是比较小的数据包也无关大碍，同时更重要的是，这样做的话，缩短了延迟，提高了带宽利用率

传统Nagle算法可以看出是一种包-停-等协议，它在未收到前一个包的确认前不会发送第二个包，
除非是“逼不得已”，而改进的Nagle算法是一种折中处理，如果未确认的不是小包，
那么第二个包可以发送出去，但是它能保证在同一个RTT内，
网络上只有一个当前连接的小包（因为如果前一个小包未被确认，不会发出第二个小包）
*/
...
{
	/*(数据包长度<当前MSS) && (TCP_NAGLE_CORK表示明确表示立即还有包要过来或者主动加塞 || 
	                          ((启用nagle算法)&&(tp->packets_out为真表示存在有发出去的数据包没有被ACK确认)
	                           &&(算法的改进在这先认为和前面ack确认相同)))*/
				 ...
				 unsigned int cur_mss, int nonagle)
{
	 ...
	 */
	if (nonagle & TCP_NAGLE_PUSH)//主动禁止nagle算法或明确最后一个包
		return 1;

	/* Don't use the nagle rule for urgent data (or for the final FIN).
	 * Nagle can be ignored during F-RTO too (see RFC4138).

		特殊包,如紧急包 带FIN的最后一个包 或者带F-RTO的包	 */
...
void tcp_send_active_reset(struct sock *sk, gfp_t priority)
{
	...
	tcp_init_nondata_skb(skb, tcp_acceptable_seq(sk),
			     TCPCB_FLAG_ACK | TCPCB_FLAG_RST); //设置RST标志
