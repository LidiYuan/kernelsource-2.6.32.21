...
int tcp_v4_connect(struct sock *sk, struct sockaddr *uaddr, int addr_len)
{
	...
	//查找路由表项，并通过变量rt记录下来
	//根据远端地址(nexthop) 原地端口(usin->sin_port) 本地地址(inet->saddr)  本地端口(inet->sport)
...
//接收到syn数据包后  执行此函数 将监听套接字加入到半连接队列中,此处并没有设置
//sock状态,sock状态是在新sock中设置
...
static struct sock *tcp_v4_hnd_req(struct sock *sk, struct sk_buff *skb)
{
	...
	//在监听队列尚未建立连接的队列中查找request_sock结构
...
int tcp_v4_do_rcv(struct sock *sk, struct sk_buff *skb)
{
	...
	if (sk->sk_state == TCP_LISTEN) 
	{
        ...
        //nsk==sk 接受到SYN 三次握手由监听端口完成
        //nsk！=sk 接受到ACK,在接收到ack后说明三次握手完成,返回通信用的新sock结构
		if (nsk != sk) {
	...
	//处理sk的状态转换 刚收到的SYN包会走到此处 此时的sk还是监听状态
	TCP_CHECK_TIMER(sk);
...
int tcp_v4_rcv(struct sk_buff *skb)
{
	...
	//获得此skb所属于的网络命名空间(用于内核级的虚拟化)
	struct net *net = dev_net(skb->dev);
	
	 //如果不是发往本地的数据包，则直接丢弃
	/*skb的pkt_type属性表示数据包的类型，其参考值由宏定义给出
		#define PACKET_HOST     0  发往本机的包
		#define PACKET_BROADCAST 1  广播包
		#define PACKET_MULTICAST 2  多播包
		#define PACKET_OTHERHOST 3  发往其他主机的包
		#define PACKET_OUTGOING 4  由本机发出的包
	*/
	if (skb->pkt_type != PACKET_HOST)
		goto discard_it;
    
	/* Count it even if it's bad  SNMP数据统计*/
	...
	//th->doff * 4表示首部的长度,检测包长是否大于从数据包获得头部长度
	...
	th = tcp_hdr(skb);//从skb中获得tcp头
	iph = ip_hdr(skb);//从skb中获得ip头
...
process:
	...
	//检查BPF规则  BSD packet filter(BPF BSD分组过滤)
	if (sk_filter(sk, skb))
		goto discard_and_relse;
	...
	if (!sock_owned_by_user(sk)) 
	{
...
#endif
		{
			...
			//尝试先将数据包放入预备队列,在预备队列处理数据包处理速度快
			if (!tcp_prequeue(sk, skb))
			//假设失败，则直接调用tcp_v4_do_rcv处理这个skb(其实也就是直接放到receive_queue中). 
				ret = tcp_v4_do_rcv(sk, skb);
		}
	}
	else	
...
static int tcp_v4_init_sock(struct sock *sk)
{
    ...
    //初始化发送和接收缓冲区大小,这些值的初始化是在tcp_init中设置的  
	sk->sk_sndbuf = sysctl_tcp_wmem[1];// /proc/sys/net/ipv4/tcp_rmem
	sk->sk_rcvbuf = sysctl_tcp_rmem[1];// /proc/sys/net/ipv4/tcp_wmem
...
struct proto tcp_prot = {
	...
	.rsk_prot		= &tcp_request_sock_ops,//tcp_ipv4.c中
