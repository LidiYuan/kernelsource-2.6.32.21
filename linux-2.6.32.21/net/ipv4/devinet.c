...
struct blocking_notifier_head inetaddr_chain = BLOCKING_NOTIFIER_INIT(inetaddr_chain); //上面宏展开后的结果就是这个 自己添加
			     ...
			     u32 pid)
{
    ...
    //如果设备上的地址链表是空的 就把它挂在后面 如果不是空的 则根据上面的判断把地址放在合适的位置
	ifa->ifa_next = *ifap;
	*ifap = ifa;
	...
	//发送NETDEV_UP事件给fib_inetaddr_notifier
		      ...
		      u32 pid)
{
    ...
    //随后，调用 inet_fill_ifaddr 将消息填充至上述缓存
    //RTM_NEWADDR 被作为 nlmsg_type 封装到了内核发送给应用程序的 netlink 消息头 nlmsghdr 中，这样用户空间的应用程序在接收后就能够根据 type 来分别处理不同类型的消息了
	...
	//在里面会调用netlink_broadcast 将上述封装完毕的 sk_buff 结构广播到 RTNLGRP_IPV4_IFADDR 这个 group
...
void __init devinet_init(void)
{
	...
	register_netdevice_notifier(&ip_netdev_notifier);//注册通知块
