...
#endif
		/* Standard target? 是否为标准的target 为NULL说明是*/
		...
		const unsigned int *underflows)
{
	...
	/*调用IPT_ENTRY_ITERATE，遍历从entry0开始的所有ipt_entry,对每一个ipt_entry调用函数check_entry_size_and_hooks，
      判断其首地址边界对其是否正确、该ipt_entry的大小是否超过了最大值、
      当前ipt_entry相对于首个ipt_entry首地址的偏移量是否与某一个newinfo->hook_entry[]的值相等*/
