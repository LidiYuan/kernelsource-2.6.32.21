					 ...
					 const struct sock *sk2))
{
	...
	if (!snum) {//如果是0，表示要系统分配
		...
		remaining = (high - low) + 1;//获得端口个数

		rand = net_random();//获得随机数
		...
		for (last = first + UDP_HTABLE_SIZE; first != last; first++) {
			 ...
			 */
			 /*
             *使用first值作为端口号，从udptable的hash表中找到hslot项，重置bitmap数组全0，
		               ...
		               size_t len)
{
	...
	//设置UDP_CORK或者设置MSG_MORE会改变udp马上将数据发送出去的行为,而是将数据进行累积,直到取消此标志
	//msg->msg_flags是由用户传递进来
	//corkreq用于指出是否使用缓冲机制
	int corkreq = up->corkflag || msg->msg_flags&MSG_MORE;
	
	int (*getfrag)(void *, char *, int, int, int, struct sk_buff *);

	//数据长度不能超过16位  报文头的len为2字节
	if (len > 0xFFFF)
		return -EMSGSIZE;
	...
	{
	    //TCP_ESTABLISHED表示已经完成了一些完整性检查,和TCP中的TCP_ESTABLISHED意义不一样
		if (sk->sk_state != TCP_ESTABLISHED)
			return -EDESTADDRREQ;

		//在内核保存的连接结构inet_sock中获得目标地址和端口
		daddr = inet->daddr;
		dport = inet->dport;
    ...
    //msg->msg_controllen不为0说明有辅助信息,用户可使用sendmsg()来发送辅助信息
	...
	//如果设置了SOCK_LOCALROUTE 或发送的时候设置了MSG_DONTROUTE
	//在或者ip选项设置了严格源路由选项,则说明目的地址或下一跳必然位于本地子网中,
	//此时需要设置tos中的RTO_ONLINK标记，表示后续查找路由时与目的地直连
...
do_append_data:
	...
	if (err)//ip_append_data失败后刷新队列
		udp_flush_pending_frames(sk);
	
	else if (!corkreq) //若没有使用缓冲机制则将数据立即传出
		//调用下面数据传输数据
...
/*以下情况是没有在hash表中找到对应的sock，在目标端口上没有侦听的udp套接字时候  会出现此情况*/
...
//udp_rcv函数是UDP模块的入口函数
//负责接收来自网络层的udp数据包的主处理程序
		...
		int bucket, int *len)
{
	...
	seq_printf(f, "%4d: %08X:%04X %08X:%04X %02X %08X:%08X %02X:%08lX %08X %5d %8d %lu %d %p %d%n",
