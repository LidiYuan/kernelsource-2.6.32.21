...
struct fib_node 
{
	...
	struct list_head	fn_alias;//挂接fib_alias(用于连接fib_alias) 其实更好的理解是这样的：虽然现在所有的路由都是同一个子网了，但是路由之间还会有其他的信息不同例如tos，路由类型，等等。所以依然存在不同的路由，所以这些都是通过fn_alias来区分。
...
struct fn_zone 
{
	struct fn_zone		*fz_next;//将活动的(路由表项不为空)zone链接在一起的指针，该链表的头部存储在fn_hash数据结构的fn_zone_list字段中
	...
	int			fz_nent; //包含的路由节总数  fib_node{}的个数 相同子网掩码的节点

	int			fz_divisor;	//表示散列表fz_hash的容量,以及散列表桶的数目
	u32			fz_hashmask;//其值为fz_divisor - 1,用来计算散列表的关键值
#define FZ_HASHMASK(fz)		((fz)->fz_hashmask)

	int			fz_order;	//网络掩码的长度，255.255.255.0的网络掩码长度为24
	__be32			fz_mask;// 子网掩码 
 ...
 */
 ...
 //包含一个由33个指针组成的向量，每个指针对应一个网络掩码并指向一个类型为fn_zone的数据结构
struct fn_hash 
{
	struct fn_zone	*fn_zones[33]; // 路由区分成33份 仔细想想，子网掩码长度是1~32，0长度掩码代表网关，那么加起来就是33，即：fn_zone[0]的掩码是0.0.0.0，fn_zone[1]是10000000.00000000.00000000.0000000这一类  等等
...
fn_new_zone(struct fn_hash *table, int z)
{
	...
	fz->fz_hash = fz_hash_alloc(fz->fz_divisor);//下一层的hash表
	if (!fz->fz_hash) 
	{
		kfree(fz);
		return NULL;
	}
	fz->fz_order = z;//子网掩码长度
	fz->fz_mask = inet_make_mask(z);//子网掩码
...
static int fn_hash_insert(struct fib_table *tb, struct fib_config *cfg)
{
    ...
    //通过key查找是否曾经创建过fib_info结构,毕竟一个唯一的路由/地址只能有一个key
	f = fib_find_node(fz, key);

	if (!f)
		//如果之前没有创建fib_node{},那么肯定就没有创建过相关的fib_alias
		fa = NULL;
	else
