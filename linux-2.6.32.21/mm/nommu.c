 ...
 */
 //使用前获取读信号量锁down_read(&current->mm->mmap_sem);
 //up_read(&current->mm->mmap_sem)
int get_user_pages( struct task_struct *tsk, //指向执行IO的任务指针 它主要目的告诉内核 当设置缓冲区时 谁负责解决页错误的问题 该参数几乎总是current
                             struct mm_struct *mm, //current->mm
	                         unsigned long start,  //用户空间缓冲区的地址 
	                         int nr_pages,  
	                         int write, //为非0 则对页有写权限 
	                         int force,
	                         struct page **pages, //如果调用成功 pages中包含了一个描述用户空间缓冲区page结构的指针列表
