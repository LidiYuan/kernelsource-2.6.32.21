...
struct vmap_area {
	...
	struct rb_node rb_node;		/* address sorted rbtree *//*红黑树节点  vmap_area_root根节点*/
	struct list_head list;		/* address sorted list *//*链表节点 vmap_area_list*/
 ...
 * vstart and vend.
 */
 //从vmap_area_root中找到合适的hole，填充vmap_area结构体，并插入到vmap_area_root红黑树中
...
retry:
	...
	if (n) 
	{
		...
		do {
			...
			{
				// 如果该区的起始虚拟地址小于要搜索的起始地址，说明该区落在了查找的起始地址范围内
 ...
 */
static void *__vmalloc_node(unsigned long size, 
                                    unsigned long align,
			                        gfp_t gfp_mask, 
			                        pgprot_t prot,
			                        int node, 
			                        void *caller)
{
	...
	//为area 分配管理page的数组 并通过伙伴系统分配物理页面,并填充page数组,与用户空间不同的是
	//此处并不是在访问时候填充page数组 而是分配时就进行了填充
