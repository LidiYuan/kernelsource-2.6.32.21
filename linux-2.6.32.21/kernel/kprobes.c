...
static int __kprobes in_kprobes_functions(unsigned long addr)
{
	...
	//kprobes把所有不能探测的函数都放在.kprobes.text section（使用__kprobes宏）中，这个段的起始地址是__kprobes_text_start和__kprobes_text_end
...
int __kprobes register_kprobe(struct kprobe *p)
{
	...
	preempt_disable();
	//检测地址是否可以被探测
	//kprobes只能用作内核函数的探测，所以在注册前必须检查探测点的地址是否是在内核地址空间中
	...
	*/
	...
	//首先要获取kprobe_mutex互斥锁，这个互斥锁用来保护kprobe_table哈希表，系统中所有已注册的kprobe实例都保存在这个哈希表中
	mutex_lock(&kprobe_mutex);
	old_p = get_kprobe(p->addr);
	if (old_p) 
	{
		//probes允许在同一个探测点注册多个kprobe，如果调用get_kprobe()能找到一个kprobe实例，说明已经在当前的探测点注册了一个kprobe，
		//这种情况下会调用register_aggr_kprobe()来处理
		ret = register_aggr_kprobe(old_p, p);
		goto out;
	}
	mutex_lock(&text_mutex);

	//将被探测位置的指令保存到kprobe结构的ainsn成员中,并且被探测位置的第一条指令保存到opcode成员中
	...
	if (!kprobes_all_disarmed && !kprobe_disabled(p))
		arch_arm_kprobe(p); //将探测点的指令替换为断点指令 
...
static int __init init_kprobes(void)
{
	...
	if (!err) //进行通知链注册 发生int3异常会进行通知  do_int3()
