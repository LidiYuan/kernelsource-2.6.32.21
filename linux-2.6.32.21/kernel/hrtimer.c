...
void hrtimer_run_queues(void)
{
	...
	//如果已经切换到高精度模式 则什么也不做 退出
	//如果返会false 说明当前处于低精度模式
	if (hrtimer_hres_active())
		return;

    //它用一个for循环遍历各个时间基准系统，查询每个hrtimer_clock_base对应红黑树的左下节点，判断它的时间是否到期
	for (index = 0; index < HRTIMER_MAX_CLOCK_BASES; index++) 
	{
		...
		while ((node = base->first)) 
		{
			...
			if (base->softirq_time.tv64 <= hrtimer_get_expires_tv64(timer))
             ...
             //到期，通过__run_hrtimer函数，对到期定时器进行处理
...
static int __sched do_nanosleep(struct hrtimer_sleeper *t, enum hrtimer_mode mode)
{
    ...
    //把hrtimer回调函数设置为hrtimer_wakeup()
    //定时器到期后 会执行回调函数hrtimer_wakeup
