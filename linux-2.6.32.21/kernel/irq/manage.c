...
void disable_irq(unsigned int irq)
{
	...
	disable_irq_nosync(irq);//增加中断所处的深度和改变中断的状态
	if (desc->action)
		synchronize_irq(irq);//使处理器处于检测终端号所对应的中断状态
}
			 ...
			 const char *devname, void *dev_id)
{
	...
	//IRQF_DISABLED：快速中断标志，对应老版本中的SA_INTERRUPT标志，
	//表明在处理本中断时屏蔽CPU所有中断（屏蔽其他中断线上的中断，本中断线上本来就是屏蔽的），
    //而在没设置此标志位的程序中，都是开中断处理的，可以进行中断嵌套。
    //嵌套与中断线优先级有关，有中断控制器控制
	/*
	...
	desc = irq_to_desc(irq);////根据中断号，找到对应的desc结构体
	if (!desc)
		return -EINVAL;
	...
	chip_bus_lock(irq, desc);
	retval = __setup_irq(irq, desc, action);//添加进action链表,添加的时候加锁
	chip_bus_sync_unlock(irq, desc);
