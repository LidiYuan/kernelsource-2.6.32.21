...
{
	if (desc->chip->mask_ack) //对于i8259a mask_and_ack_8259A()
...
void handle_level_irq(unsigned int irq, struct irq_desc *desc)
{
	...
	//屏蔽响应此中断  屏蔽只是在当前cpu屏蔽 但在多cpu情况下 其他cpu可能还会响应此中断
    ...
    //调用通过request_irq设置的回调处理用户注册的流程
	action_ret = handle_IRQ_event(irq, action);//遍历链表 来处理用户注册的服务程序
	...
	//中断历程处理完  标志要释放此中断的使用权
	desc->status &= ~IRQ_INPROGRESS;

	if (unlikely(desc->status & IRQ_ONESHOT))
		desc->status |= IRQ_MASKED;
	else if (!(desc->status & IRQ_DISABLED) && desc->chip->unmask)
		desc->chip->unmask(irq);//enable_8259A_irq() 取消中断的屏蔽
out_unlock:
	spin_unlock(&desc->lock);
}
