...
static pte_t *__init page_table_kmap_check(pte_t *pte, pmd_t *pmd,unsigned long vaddr, pte_t *lastpte)
{
...
static void __init page_table_range_init(unsigned long start, unsigned long end, pgd_t *pgd_base)
{
	...
	pgd_idx = pgd_index(vaddr);//全局页目录索引

	pmd_idx = pmd_index(vaddr);//中间页目录索引
	
	pgd = pgd_base + pgd_idx;//找到全局目录表中对应的一项

    //PTRS_PER_PGD 全局页目录的最大表项个数 此层循环 循环所有的全局页目录表项
...
static void __init kmap_init(void)
{
	 ...
	 */
	kmap_vstart = __fix_to_virt(FIX_KMAP_BEGIN);/*得到固定映射区的临时映射区的虚拟地址起始地址*/  
...
{
	//pgd_base保存页全局目录表的起始地址swapper_pg_dir
	pgd_t *pgd_base = swapper_pg_dir;
    //永久内存映射区的页表初始 
				 ...
				 unsigned long bootmap)
{
	...
	bootmap_size = init_bootmem_node(NODE_DATA(nodeid),bootmap >> PAGE_SHIFT,start_pfn, end_pfn);
...
void __init setup_bootmem_allocator(void)
{
	...
	//遍历每个节点 为每个节点初始化pg_data_t{}->bdata
 ...
 */
 //该函数建立完整的页表，注意起始的8MB已经被head_32.S映射了
