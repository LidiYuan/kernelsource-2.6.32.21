			     ...
			     u32 *pnr_map)
{
	...
	//确保内存区段都是正确的 start+size 不能小于区段的开始start
	for (i = 0; i < old_nr; i++)
...
static unsigned long __init e820_end_pfn(unsigned long limit_pfn, unsigned type)
{
  ...
  ///*对e820中所有的内存块,其中e820为从bios中探测到的页面数存放处*/
	for (i = 0; i < e820.nr_map; i++) {
		...
		start_pfn = ei->addr >> PAGE_SHIFT;///*起始地址对应的页面帧号*/
		end_pfn = (ei->addr + ei->size) >> PAGE_SHIFT;	/*结束物理地址对应的页面帧号*/
...
//setup_memory_map()->default_machine_specific_memory_setup()
//将boot_params.e820_map 内存段区域拷贝到e820表
...
void __init setup_memory_map(void)
{
	...
	who = x86_init.resources.memory_setup(); //调用函数   default_machine_specific_memory_setup()
	memcpy(&e820_saved, &e820, sizeof(struct e820map));
	printk(KERN_INFO "BIOS-provided physical RAM map:\n");

	//通过dmesg | grep e820显示的内容
	e820_print_map(who);
}
